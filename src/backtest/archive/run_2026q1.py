"""
2026 Q1 Backtest: Full Pipeline
================================
Runs Phase 1 (OI analysis) -> Phase 2 (stock sim) -> Phase 3 (F&O)
for January-March 2026 trading dates.

Config: Grade A+B | 1:3 R:R (10% SL / 30% Target) | 10L Capital | Max 5L/trade | 5-min buffer

Usage:
  python -m src.backtest.run_2026q1
"""
...
# ── Config ────────────────────────────────────────────────────────────
TRADING_DATES = [
    # January 2026 (21 days) — Jan 26 Republic Day excluded
    ...
    # February 2026 (20 days) — no holidays (Mahashivratri falls on Sunday)
    "2026-02-02", "2026-02-03", "2026-02-04", "2026-02-05", "2026-02-06",
    "2026-02-09", "2026-02-10", "2026-02-11", "2026-02-12", "2026-02-13",
    "2026-02-16", "2026-02-17", "2026-02-18", "2026-02-19", "2026-02-20",
    "2026-02-23", "2026-02-24", "2026-02-25", "2026-02-26", "2026-02-27",
    # March 2026 (4 days up to Mar 6) — Mar 3 Holi excluded
...
SL_PCT = 10.0
TARGET_PCT = 30.0            # 1:3 R:R on option premium
ACCEPTED_GRADES = ["A", "B"]
RSI_OVERBOUGHT = 80.0
ENTRY_OFFSET = 5
...
logger = logging.getLogger("Q1-2026")


def apply_overrides():
    strategy_config.REWARD_RATIO = 3.0
...
async def run():
    ...
    run_dir = RESULTS_DIR / f"q1_2026_{ts}"
    run_dir.mkdir(parents=True, exist_ok=True)
    logger.info(f"Results directory: {run_dir}")

    with open(run_dir / "backtest_config.json", "w") as f:
        json.dump({
            "name": "q1_2026_gradeAB_rr3",
            "description": "Q1 2026 (Jan-Mar): Grade A+B, 1:3 R:R (10% SL / 30% Target), "
                           "10L capital, max 5L/trade, 5-candle buffer, RSI<80 filter, "
                           "NIFTY VWAP block, pure OI magnitude, TRADING_END_TIME=14:00",
            "trading_dates": TRADING_DATES,
            "num_trading_days": len(TRADING_DATES),
            "strategy": {
                "reward_ratio": 3.0,
                "sl_pct_on_premium": SL_PCT,
                "target_pct_on_premium": TARGET_PCT,
                "entry_offset_candles": ENTRY_OFFSET,
            ...
            "capital": {
                ...
                "max_per_trade": MAX_CAPITAL_PER_TRADE,
            },
            "analysis": {
                "oi_weight_blend": 1.0,
    ...
    logger.info(f"Phase 1: Analyzing {len(TRADING_DATES)} dates ({TRADING_DATES[0]} -> {TRADING_DATES[-1]})")
    ...
    if not all_signals:
        logger.info("No Grade A+B signals found for Q1 2026")
        with open(run_dir / "q1_2026_summary.json", "w") as f:
    ...
    # ── Simulate with 10L capital, max 5L/trade, 1:3 R:R ─────────────
    ...
    print(f"\n{'=' * 100}")
    print(f"  Q1 2026 | Grade A+B | 1:3 R:R (10% SL / 30% Target) | 10L Capital | Max 5L/trade | 5-candle buffer")
    print(f"{'=' * 100}")
    print(f"  Trading dates    : {len(TRADING_DATES)} days ({TRADING_DATES[0]} -> {TRADING_DATES[-1]})")
    ...
    # ── Save CSV ──────────────────────────────────────────────────────
    csv_path = run_dir / "q1_2026_gradeAB_rr3_trades.csv"
    if rows:
    ...
    summary = {
        "strategy": "Grade A+B | 1:3 R:R (10% SL / 30% Target) | 5-candle buffer",
        "trading_dates": TRADING_DATES,
        "num_trading_days": len(TRADING_DATES),
        "starting_capital": STARTING_CAPITAL,
