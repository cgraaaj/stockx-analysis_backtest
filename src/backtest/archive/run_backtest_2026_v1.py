...
BACKTEST_CONFIG = {
    ...
    "description": (
        ...
        "1:2 R:R, NIFTY VWAP trend (no lookahead, cutoff at signal time), "
    ...
    "batch_size": 10,  # API URL length limit prevents batch_size=50
...
async def run():
    ...
    if market_svc.is_enabled and date_index_contexts:
        ...
        logger.info(f"Market context filter (per-date): {before} → {after} predictions")
...
def _filter_preds_per_date(preds, date_index_contexts, market_svc):
    """Apply market context filtering using each prediction's own date context."""
    filtered = []
    for pred in preds:
        ...
        trend = idx_ctx.trend
        conflict = MarketContextService._is_conflicting(pred, trend)
        if not conflict:
            filtered.append(pred)
        else:
            mode = market_context_config.FILTER_MODE
            if mode == "block":
                logger.debug(f"Blocked {pred.stock} ({pred.option_type}) on {pred_date} — "
                             f"{idx_name} trend={trend}")
            else:
                from src.analysis.config.settings import Grade
...
def _rebuild_entries(filtered, original_entries):
    kept = {(p.stock, p.timestamp) for p in filtered}
    rebuilt = []
    for entry in original_entries:
        kept_data = [
            sd for sd in entry.get("stock_data", [])
            if (sd["stock"], sd["time_stamp"]) in kept
        ]
        if kept_data:
