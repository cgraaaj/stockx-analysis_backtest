...
# ── Config ────────────────────────────────────────────────────────────
# All weekdays Jan-Mar 2026. Non-trading days (holidays) are
# automatically skipped when the API returns no data.
TRADING_DATES = [
    # January 2026
...
async def run():
    ...
    print(f"\n{'=' * 90}")
    print(f"  Q1 2026 (Jan-Mar) | Grade A+B | 10% SL / 30% Target | 10L Capital | Max 5L/trade | 5min buffer")
    print(f"{'=' * 90}")
    print(f"  Trading dates    : {len(TRADING_DATES)} weekdays (Jan-Mar 2026)")
    ...
    # ── Save CSV ──────────────────────────────────────────────────────
    csv_path = run_dir / "gradeAB_10l_max5l_5minbuf_trades.csv"
    if rows:
